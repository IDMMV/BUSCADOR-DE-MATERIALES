...
function doGet(e) {
  try {
    ...
    if (action === 'getImage') {
      ...
      if (!fileId) throw new Error('Falta fileId para recuperar la imagen.');
...
function setup() {
  ...
  if (currentId) {
    try { return 'Base vinculada: ' + SpreadsheetApp.openById(currentId).getUrl(); }
    catch (_) { props.deleteProperty('SPREADSHEET_ID'); }
  }
...
function readStockFromSheets_() {
  const ss=getSpreadsheet_();
  const stockSheet=ss.getSheetByName('Stock'), metaSheet=ss.getSheetByName('StockMeta'), stockRows=[];
  let stockMeta={};
  if (stockSheet && stockSheet.getLastRow()>1) {
    const vals=stockSheet.getRange(2,1,stockSheet.getLastRow()-1,7).getValues();
    vals.forEach(r=>{ if(!r[0])return; stockRows.push({material:String(r[0]).replace(/\.0$/,''),centro:String(r[1]),almacen:String(r[2]),lote:String(r[3]),libre:Number(r[4])||0,unidad:String(r[5]),texto:String(r[6])}); });
  }
...
function normalizeDate_(value) {
  ...
  const text=String(value).trim(), iso=text.match(/^(\d{4})-(\d{2})-(\d{2})/), latam=text.match(/^(\d{1,2})[\/-](\d{1,2})[\/-](\d{4})/);
...
function normalizeDriveImageUrl_(url) {
  ...
  const text=String(url||'').trim();
  return text&&text!=='SIN IMAGEN'?text:'';
}

function readMaterialsFromSheets_() {
  const ss=getSpreadsheet_(), sh=ss.getSheetByName('Materiales');
  if(!sh||sh.getLastRow()<=1)return[];
  const lastCol=Math.max(8,sh.getLastColumn()), values=sh.getRange(2,1,sh.getLastRow()-1,lastCol).getValues(), out=[];
  values.forEach(r=>{
    const code=String(r[0]||'').replace(/\.0$/,'').trim(); if(!code)return;
    const aliases=String(r[4]||'').split('|').map(x=>x.trim().toUpperCase()).filter(Boolean);
    const rawImage=String(r[6]||'').trim(), imageUrl=normalizeDriveImageUrl_(rawImage), imageDriveId=extractDriveFileId_(rawImage||imageUrl);
    out.push({code:code,description:String(r[1]||'').trim(),unit:String(r[2]||'UND').trim().toUpperCase(),priority:String(r[3]||'').trim().toUpperCase(),aliases:aliases,image:imageUrl,imageStatus:String(r[5]||'').trim().toUpperCase(),imageDriveUrl:imageUrl,imageDriveId:imageDriveId});
  });
  return out;
}

function mergeMaterialsWithDriveImages_(backupMaterials,sheetMaterials) {
  const backup=Array.isArray(backupMaterials)?backupMaterials:[], byCode={};
  sheetMaterials.forEach(m=>{byCode[String(m.code)]=m;});
  const merged=backup.map(m=>{
    const sheet=byCode[String(m.code)]; if(!sheet)return m;
    ...
    if(sheet.image){next.image=sheet.image;next.imageDriveUrl=sheet.imageDriveUrl;next.imageDriveId=sheet.imageDriveId;next.imageStatus='SI';}
    return next;
  });
  const existing=new Set(merged.map(m=>String(m.code)));
  sheetMaterials.forEach(m=>{if(!existing.has(String(m.code)))merged.push({code:m.code,description:m.description,unit:m.unit||'UND',priority:m.priority||'',aliases:m.aliases||[],image:m.image||'',imageDriveUrl:m.imageDriveUrl||'',imageDriveId:m.imageDriveId||'',imageStatus:m.imageStatus||''});});
  return merged;
}
...
function writeSheets_(data) {
  ...
  writeTable_(ss,'Materiales',['MATRICULA','DESCRIPCION','UNIDAD','PRIORIDAD','ALIAS','IMAGEN_ESTADO','LINK_IMAGEN_DRIVE','ACTUALIZADO'],(data.app&&data.app.materials||[]).map(m=>{
    const imgInfo=imgMap[m.code]||{}, link=imgInfo.thumbnail||normalizeDriveImageUrl_(imgInfo.url||'')||(m.image&&!String(m.image).startsWith('data:')?normalizeDriveImageUrl_(m.image):'SIN IMAGEN'), hasImage=!!link&&link!=='SIN IMAGEN';
  ...
  const history=(data.app&&data.app.orderHistory||[]), detailRows=[];
...
function writeTable_(ss,name,headers,rows) {
  let sh=ss.getSheetByName(name); if(!sh)sh=ss.insertSheet(name); sh.clearContents();
...
function saveImages_(materials) {
  const folder=getFolder_(), it=folder.getFoldersByName('imagenes_materiales'), imgs=it.hasNext()?it.next():folder.createFolder('imagenes_materiales'), imgMap={};
  const matsWithImage=materials.filter(m=>m&&m.image&&String(m.image).startsWith('data:image/'));
  matsWithImage.forEach(m=>{
    try{
      const parts=m.image.split(','), mimeMatch=parts[0].match(/data:(.*?);/); if(!mimeMatch)return;
      const mime=mimeMatch[1], bytes=Utilities.base64Decode(parts[1]), ext=mime.indexOf('webp')>=0?'webp':'jpg', name=m.code+'.'+ext;
      const olds=imgs.getFilesByName(name); let file=null;
      if(olds.hasNext())file=olds.next();else file=imgs.createFile(Utilities.newBlob(bytes,mime,name));
      try{file.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);}catch(_){ }
      const id=file.getId(), thumbnail='https://drive.google.com/thumbnail?id='+id+'&sz=w1200';
